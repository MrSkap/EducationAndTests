...
public class CarIdentifierTests
{
    ...
    public CarIdentifierTests()
    {
        _mediatorMock.Setup(x => x.Send(It.IsAny<CarIdentificationContext>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((CarIdentificationContext x, CancellationToken arg2) => new CarIdentificationResult(){Type = x.Car.Model, Match = true});
    }
    ...
    public async Task IdentifyCars_Statistic_Correct()
    {
        ...
        foreach (var car in cars.Where(car => !myStatistic.TryAdd(car.Model, 1)))
        {
            myStatistic[car.Model]++;
        }

        foreach (var model in myStatistic.Keys)
        {
            statistic[model].Should().Be(myStatistic[model]);
        }
    }
